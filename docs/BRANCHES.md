...
| 3 | `v1` | branch | Current active product line under `versions/v1/`. |
...
The current `v1.0.1` line was verified with:

```bash
npm test
npm --prefix versions/v1 test
npm run test:remote
git diff --check
```

Live E2E verification requires TradingView Desktop to be relaunched with CDP on port `9222`.
